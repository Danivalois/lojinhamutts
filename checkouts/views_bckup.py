...
def checkout_view(request):
    if request.method == "POST":
        ...
        for service in ["SEDEX", "PAC", "MINIPAC", "JADLOG_EXP"]:
    ...
    if request.method == "POST":
        ...
        if is_valid and flag_empty:
          ...
          if is_valid == False and flag_empty == False:
              message="Os campos Nome, Número da casa, E-mail e/ou Telefone são obrigatórios.  CPF Inválido."
          else:       
              if flag_empty == False:
                message="Os campos Nome, Número da casa, E-mail e/ou Telefone são obrigatórios."
              else:
...
def process_payment(request):
    if request.method == "POST":
        try:
            ...
            unit_price = round(float(request.POST.get("product_unit_price", 0)), 2)
