...
def send_order_emails(order):
    ...
    try:
        ...
        if order.order_product.product_image_url: #Always check if the product has images
            try:
                image_url = (
                ...
                # Generate unique CID
                image_cid = "my_test_image" # Hard coded this to be sure

                # Get the file path from the image field
                image_path = order.order_product.product_image_url.path #Get the image path

                # Determine MIME type
                mime_type, _ = mimetypes.guess_type(image_path) #Now it is correct
                if mime_type is None:
                    mime_type = 'application/octet-stream'  # Fallback
                with open(image_path, 'rb') as img_file: #Get image file
                    img_data = img_file.read()
                msg = EmailMultiAlternatives( #Use arguments correctly

                    subject = "Subject here",  # Subject
                    body = "Body Here",  # Body, use and empty string ""
                    from_email = settings.DEFAULT_FROM_EMAIL, #From email
                    to = ["antonio.leitao@gmail.com"]  #To email
                )

                html_content_with_cid = render_to_string('emails/admin_order_email.html',{'image_cid': image_cid})

                msg.attach_alternative(html_content_with_cid, "text/html")
                msg.mixed_subtype = 'related'
                image_path = Path(image_path)
                msg.attach(image_path.name, img_data, mime_type) #Take name from file.
            ...
            print("No image to send") #For debugging
